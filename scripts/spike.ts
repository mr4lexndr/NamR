...
const first = process.argv[2] ?? 'Bożena';
const last = process.argv[3] ?? 'Dąbrowa';
...
console.log(`  profile ${countPoints(r.polys)} pts | arc segments ${arcSegments(DEFAULT_TAG.sweep, 25)}`);
