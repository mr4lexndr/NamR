...
def render(tri, out, az, el, size=(760,520), fov=38.0, margin=1.12):
    v = tri.reshape(-1,3)
    c = (v.min(0)+v.max(0))/2
    tri = tri - c
    ...
    eye = np.array([dist*np.cos(el)*np.sin(az), dist*np.sin(el), dist*np.cos(el)*np.cos(az)])
    ...
    L = np.array([-0.4,0.5,0.76]); L/=np.linalg.norm(L)
